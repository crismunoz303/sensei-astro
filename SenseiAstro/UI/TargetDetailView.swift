...
struct TargetDetailView: View {
    ...
    private var detailGrid: some View {
        LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 9) {
            MetricView(title: "CAPTURE WINDOW", value: "\(plan.start.astroTime)-\(plan.end.astroTime)", detail: "Peak \(plan.bestTime.astroTime)")
            ...
            MetricView(title: "EXPOSURE", value: "10 SEC x \(plan.acceptedFrames)", detail: "\(plan.integrationMinutes) min accepted")
    ...
    private var sequencePanel: some View {
        AstroPanel {
            VStack(alignment: .leading, spacing: 10) {
                ...
                step(5, "Use 10-second sub-exposures. Begin near \(plan.start.astroTime) and collect \(plan.integrationMinutes) minutes of accepted frames.")
